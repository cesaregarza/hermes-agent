...
Features:
    ...
    - Session context forwarding: per-server ``forward_session_context``
      ...
      ``_meta`` under ``com.nousresearch.hermes/*``. Individual values may be
      empty, but an unbound value, an all-empty identity, an unavailable routed
      profile privacy policy, or a failed privacy transformation omits the
      entire meta block. Values never fall back to process environment
      variables, and MCP resource, prompt, and discovery operations carry no
      session identity.
      When ``privacy.redact_pii`` is enabled on an eligible platform, the
      outgoing copy uses deterministic pseudonyms for chat/user IDs, the full
      thread ID, the message ID (including WhatsApp Cloud WAMIDs), and the
      session key; platform and session ID remain raw. Only the outgoing copy
      changes; task-local routing values remain raw. With redaction disabled or
      on an ineligible platform, all values remain raw. These stable, linkable
      values are pseudonyms, not anonymity. Independently of this MCP opt-in,
      binding a gateway session ID exposes ``HERMES_SESSION_ID`` to
